...
def draw_tracked_circles(frame, trackers_data, initial_radius, initial_bbox_size, label="Consensus"):
    ...
    # Draw consensus circle if we have multiple trackers
    if len(successful) > 0:
        # Weighted average of all centers
        consensus_x = int(sum(c[0] * w for c, w in zip(all_centers, weights)))
        consensus_y = int(sum(c[1] * w for c, w in zip(all_centers, weights)))
        consensus_center = (consensus_x, consensus_y)
        
        # Weighted average of radii
        consensus_radius = int(sum(r * w for r, w in zip(all_radii, weights)))
...
if __name__ == '__main__':
    ...
    # Read video
    video = cv2.VideoCapture("videos/Microscopy1.mp4")
    ...
    while True:
        ...
        frame = draw_tracked_circles(frame, trackers_data, CIRCLE_RADIUS, initial_bbox_size, CIRCLE_LABEL)
        
        # Display info
        active_count = sum([ok1, ok2, ok3])
        cv2.putText(frame, f"Trackers: {active_count}/3 active | Radius: {CIRCLE_RADIUS}px", 
